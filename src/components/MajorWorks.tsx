...
export const MajorWorks: React.FC<MajorWorksProps> = ({
  ...
  return (
    <section className="py-10 lg:py-14 bg-[#f2f8f3]">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3 sm:gap-4">
          {MAJOR_WORKS.map((work) => (
            <div
              ...
              className="bg-white rounded-2xl p-4 sm:p-5 border border-slate-100 shadow-xs hover:shadow-md hover:border-emerald-300 transition flex flex-col justify-between items-center text-center group cursor-pointer"
            >
              {/* Circular Light Green Icon Box */}
              <div className="w-12 h-12 sm:w-14 sm:h-14 rounded-full bg-[#e2f2e5] flex items-center justify-center text-[#1a702b] mb-3 group-hover:scale-110 transition shrink-0">
                {getIcon(work.icon)}
              </div>

              {/* Title & Description */}
              <div className="space-y-1 mb-3 flex-1 flex flex-col justify-start">
                <h3 className="font-extrabold text-slate-900 text-sm sm:text-base font-['Noto_Sans_Devanagari'] group-hover:text-[#1a702b] transition leading-snug">
                  {language === 'hi' ? work.title : work.titleEn}
                </h3>
                <p className="text-[11px] sm:text-xs text-slate-600 leading-relaxed font-medium font-['Noto_Sans_Devanagari']">
                  {language === 'hi' ? work.description : work.descriptionEn}
                </p>
              </div>

              {/* Action Link: "और जानें ->" */}
              <div className="flex items-center justify-center gap-1 text-xs font-extrabold text-[#1a702b] group-hover:text-[#145b22] mt-auto pt-1">
